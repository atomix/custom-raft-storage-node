...
func (r *ActiveRole) isLogUpToDate(lastIndex int64, lastTerm int64, request interface{}) bool {
	...
	if lastEntry == nil {
		log.Debug("Accepted %v: candidate's log is up-to-date", request)
		return true
	}
	...
	if lastTerm < lastEntry.Entry.Term {
		log.Debug("Rejected %v: candidate's last log entry (%d) is at a lower term than the local log (%d)", request, lastTerm, lastEntry.Entry.Term)
		return false
	}
...
func (r *ActiveRole) handleVote(ctx context.Context, request *VoteRequest) (*VoteResponse, error) {
	if request.Term < r.raft.term {
		...
		// vote for the candidate. We want to vote for candidates that are at least
		// as up to date as us.
		log.Debug("Rejected %v: candidate's term is less than the current term", request)
		return &VoteResponse{
			Status: ResponseStatus_OK,
			Term:   r.raft.term,
			Voted:  false,
		...
		log.Debug("Rejected %v: leader already exists", request)
		return &VoteResponse{
			Status: ResponseStatus_OK,
			Term:   r.raft.term,
			Voted:  false,
		...
		log.Debug("Rejected %v: candidate is not known to the local member", request)
		return &VoteResponse{
			Status: ResponseStatus_OK,
			Term:   r.raft.term,
			Voted:  false,
		...
		log.Debug("Accepted %v: already voted for %s", request, request.Candidate)
		return &VoteResponse{
			Status: ResponseStatus_OK,
			Term:   r.raft.term,
			Voted:  true,
		...
		log.Debug("Rejected %v: already voted for %s", request, r.raft.lastVotedFor)
		return &VoteResponse{
			Status: ResponseStatus_OK,
			Term:   r.raft.term,
			Voted:  false,
