...
func (r *ActiveRole) handlePoll(ctx context.Context, request *PollRequest) (*PollResponse, error) {
	...
	if request.Term < r.raft.term {
		log.WithField("memberID", r.raft.cluster.member).Debugf("Rejected %v: candidate's term is less than the current term", request)
		return &PollResponse{
...
func (r *ActiveRole) isLogUpToDate(lastIndex int64, lastTerm int64, request interface{}) bool {
	...
	if lastEntry == nil {
		log.WithField("memberID", r.raft.cluster.member).Debugf("Accepted %v: candidate's log is up-to-date", request)
		return true
	}
	...
	if lastTerm < lastEntry.Entry.Term {
		log.WithField("memberID", r.raft.cluster.member).Debugf("Rejected %v: candidate's last log entry (%d) is at a lower term than the local log (%d)", request, lastTerm, lastEntry.Entry.Term)
		return false
	}
	...
	if lastTerm == lastEntry.Entry.Term && lastIndex < lastEntry.Index {
		log.WithField("memberID", r.raft.cluster.member).Debugf("Rejected %v: candidate's last log entry (%d) is at a lower index than the local log (%d)", request, lastIndex, lastEntry.Index)
		return false
	}
...
func (r *ActiveRole) handleVote(ctx context.Context, request *VoteRequest) (*VoteResponse, error) {
	if request.Term < r.raft.term {
		...
		log.WithField("memberID", r.raft.cluster.member).Debugf("Rejected %v: leader already exists", request)
		return &VoteResponse{
			Status: ResponseStatus_OK,
			Term:   r.raft.term,
			Voted:  false,
		...
		log.WithField("memberID", r.raft.cluster.member).Debugf("Rejected %v: candidate is not known to the local member", request)
		return &VoteResponse{
			Status: ResponseStatus_OK,
			Term:   r.raft.term,
			Voted:  false,
		...
		log.WithField("memberID", r.raft.cluster.member).Debugf("Accepted %v: already voted for %s", request, request.Candidate)
		return &VoteResponse{
			Status: ResponseStatus_OK,
			Term:   r.raft.term,
			Voted:  true,
		...
		log.WithField("memberID", r.raft.cluster.member).Debugf("Rejected %v: already voted for %s", request, r.raft.lastVotedFor)
		return &VoteResponse{
			Status: ResponseStatus_OK,
			Term:   r.raft.term,
			Voted:  false,
